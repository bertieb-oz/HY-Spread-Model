...
def engineer_features(df, variable_flags, original_names):
    ...
    Returns:
        ...
        feature_cols: list of derived feature column names (for regression)
    ...
    return feat, feature_cols, feature_labels


# ─────────────────────────────────────────────────────────────
# Regression engine
# ─────────────────────────────────────────────────────────────
def run_regression(feat_df, feature_cols, lookback):
...
def main():
    ...
    **Why asymmetric thresholds?**
    ...
    # Feature engineering
    feat_df, feature_cols, feature_labels = engineer_features(raw_df, variable_flags, original_names)
    ...
    with st.spinner("Running rolling regression..."):
        reg_result = run_regression(feat_df, feature_cols, lookback)
    ...
    with tab4:
        st.plotly_chart(chart_coefficients(model_stats, feature_cols, feature_labels), use_container_width=True)

    st.markdown("---")

    # ── Section 4: Monthly Attribution ──
    with st.expander("🔍 Monthly Attribution Analysis", expanded=False):
        attrib = monthly_attribution(results, coef_df, feature_cols, feature_labels)
