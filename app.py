...
def load_data(uploaded_file):
    ...
    for skip in [0, 1, 2]:
        try:
            ...
            # Check if second row looks like Bloomberg tickers — skip it
    ...
    if "date" not in col_map or "hy_spread" not in col_map:
        st.error("❌ Could not find required columns. Please ensure your file has a 'Date' column and a 'US High Yield Spread' column.")
        return None, None

    missing = [k for k in EXPECTED_COLS if k not in col_map]
    if missing:
        st.error(f"❌ Missing columns: {missing}. Found: {list(col_map.keys())}")
        return None, None
...
def generate_excel_report(results, model_stats, raw_df, feat_df, attrib_df):
    ...
    ws4.append(["R²", round(model_stats["r2"], 6)])
    ws4.append(["Adjusted R²", round(model_stats["adj_r2"], 6)])
